    ...
    def generate_script(self, args):
        ...
        if args.inference:
            ...
            if args.lr is not None:
                base += f" --lr {args.lr}"
            if args.lr_opts is not None:
                base += f" --lr_opts {args.lr_opts}"
            ...
            if args.no_normalize_weight:
                base += " --no_normalize_weight"
            if args.opt_hidden_dim is not None:
                base += f" --opt_hidden_dim {args.opt_hidden_dim}"
            if args.output_norm_method is not None:
                base += f" --output_norm_method {args.output_norm_method}"
...
def main():
    ...
    parser.add_argument('--lr', type=float,
                        help='Learning rate')
    parser.add_argument('--lr-opts', type=float,
                        help='Learning rate for optimizer parameters')
    ...
    parser.add_argument('--opt-hidden-dim', type=int,
                        help='Hidden dimension of optimizer')
    parser.add_argument('--output-norm-method', type=str,
                        choices=['none', 'mean_std', 'affine'],
                        help='Output normalization method')
